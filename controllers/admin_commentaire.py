...
def admin_article_details():
    ...
    sql = '''
    SELECT c.commentaire, c.date_publication, u.login
    FROM commentaire c
...
def admin_comment_add():
    ...
    sql = '''
    INSERT INTO commentaire (commentaire, date_publication, utilisateur_id, cle_usb_id)
    VALUES (%s, NOW(), %s, %s)
    '''
    mycursor.execute(sql, (commentaire, id_utilisateur, id_article))
    get_db().commit()
    return redirect('/admin/article/commentaires?id_article='+id_article)
...
def admin_comment_valider():
    ...
    UPDATE commentaire
    SET est_valide = 1
    WHERE cle_usb_id = %s AND est_valide = 0
    '''
    mycursor.execute(sql, (id_article,))
    get_db().commit()
    return redirect('/admin/article/commentaires?id_article='+id_article)